...
async function ex4() {
	queryBuilder
		.addMatchNode(["Movie"], null, "t")
		.addWhere("t.tagline =~ $str", {str: '.*have.*'})
		.addReturn("t");

	console.log(queryBuilder.getQuery());
	const res = await neo.executeQuery(queryBuilder);
	console.log(res[0][0].properties.title);
}

async function ex5() {
	queryBuilder
		.addMatchNode(["Movie"], null, "t")
		.addWhere("size(split(t.tagline, ' ')) = $nb", {nb: 3})
		.addReturn("t");
...
async function ex6() {
	const pattern = queryBuilder.createPattern()
		.addNode(["Person"], null, "p")
		.addRelation(["ACTED_IN"])
		.addNode(["Movie"], {title: 'Speed Racer'});

	const not = queryBuilder.createPattern();
	not.addNode(null, null, "p")
		.addRelation()
		.addNode(["Movie"], {title: 'V for Vendetta'});

	queryBuilder
		.addMatchPattern(pattern)
		.addWherePattern(not, "NOT")
		.addReturn("p");
...
async function ex7() {
	const pattern = queryBuilder.createPattern()
		.addNode(["Movie"], {title: "The Matrix Reloaded"}, "m")
		.addRelation(["DIRECTED"])
		.addNode(null, null, "p")
		.addRelation(["DIRECTED"])
		.addNode(["Movie"], null, "r");
...
async function ex8() {
	...
	queryBuilder
		.addMatchPattern(pattern)
		.addWhere("toLower(m.title) =~ $title", {title: "the.*"})
		.addDistinct()
		.addReturn("r");
...
async function ex9() {
	const pattern = queryBuilder.createPattern()
		.addNode(["Person"], null, "r")
		.addRelation(["WROTE"])
		.addNode(["Movie"], null, "m")
		.addRelation(["PRODUCED"])
		.addNode(null, {name: "Stefan Arndt"}, "p");
	...
	queryBuilder
		...
		console.log(queryBuilder.getRunnableQuery());
		const res = await neo.executeQuery(queryBuilder);
		console.log(res[0][0].properties.name);
}

async function ex10() {
	const pattern = queryBuilder.createPattern()
		.addNode(["Person"], {name: "J.T. Walsh"}, "p")
		.addRelation()
		.addNode(["Movie"])
		.addRelation()
		.addNode(["Person"], null, "n");
	...
	const not = queryBuilder.createPattern()
		...
		.addNode(["Movie"])
		.addRelation()
		.addNode(["Person"], {name: "Danny DeVito"});
...
async function ex11() {
	const pattern = queryBuilder.createPattern()
		.addNode(["Person"], null, "p")
		.addRelation()
		.addNode(["Movie"], {title: "The Da Vinci Code"});

	const not = queryBuilder.createPattern()
		.addNode(null, null, "p")
		.addRelation()
		.addNode(["Movie"], {title: "Apollo 13"});
...
async function ex12() {
	const pattern = queryBuilder.createPattern()
		.addNode(["Person"], {name: "Angela Scope"})
		.addRelation(null, null, null, null, "*")
		.addNode(["Person"], {name: "Wil Wheaton"});

	queryBuilder
		.shortestPath(pattern, "p")
		.addReturn("p");
